...
window.htmlTemplates.forums = '''
  <% _.each( data.forums, function( f ){ %>
    <div style="float:left">
      ...
      <span class="forum-item" ng-click="selectForum('<%= f.id %>', '<%= data.id %>', $event)"><%= f.forum_name %></span>
    </div>
  <% }); %>
'''
...
window.htmlTemplates.postBody = '''
  ...
  <div class="messageMeta">
    ...
    <div class="publicControls">
      ...
      <% } %>
      <a class="item">Like</a>
      <a title="Reply, quoting this message" class="item" ng-click="replyToPost('<%= data.topic %>', '<%= data.id %>', $event)">Reply</a>
    </div>
  </div>
'''